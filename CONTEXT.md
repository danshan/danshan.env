...
**Local Shell State**: Shell 或插件在本机运行时持久化的状态, 不属于仓库声明的 Desired State.

**Raycast Ignore File**: 位于 HOME 根目录的 `.rayignore`, 使用 glob pattern 从 Raycast File Search 中排除匹配的文件. 本仓库通过 `raycast` Stow package 管理该文件.
