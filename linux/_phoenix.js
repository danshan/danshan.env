 ...
 */
Phoenix.set({
  // if set true Phoenix will run completely in the background, 
  // this also removes the status bar menu, defaults to false
  'daemon': false,
  // if set true Phoenix will automatically open at login, 
  // defaults to false if no value has been previously set
  'openAtLogin': true
});
...
keys.push(new Key('s', alt, function () { callApp('IntelliJ IDEA Ultimate'); }));
//keys.push(new Key('e', alt, function() { callApp('Sublime Text'); }));
keys.push(new Key('e', alt, function () { callApp('VSCode'); }));
...
keys.push(new Key('a', alt, function () { callApp('Mail'); }));
